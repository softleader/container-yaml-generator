...
function loadService(file, options) {
  ...
  if (options.publishAll) {
    ...
    style[name].ports.push(options.publishAll + ":" + options.publishAll);
    if (!style[name].environment['DEVOPS_OPTS']) {
      style[name].environment['DEVOPS_OPTS'] = '';
    }
    style[name].environment['DEVOPS_OPTS'] = style[name].environment['DEVOPS_OPTS'] + ' -Dserver.port=' + options.publishAll;
    options.publishAll = options.publishAll + 1;
  }
