...
exports.generate = options => {
  ...
  let devPortStart = options.devIpAddress ? options.devPort : undefined
  options.devPortPublished = 0;
  options.args
  ...
  if (devPortStart && options.devPortPublished > 0) {
    console.log("Auto publish port from [%s] to [%s]", devPortStart, devPortStart + options.devPortPublished)
  }
